...
- indexOf는 얕은 비교 === 동등연산자를 사용하여 비교함 객체타입은 참조값을 통해서 비교가 되기 때문에 프로퍼티를 기준으로는 index를 찾기 힘듬
- 그래서 findIndex를 사용하는 것임 콜백함수를 이용해서 item의 name이라는 프로퍼티의 값을 비교하는것이기 때문에 정확한 위치를 찾을 수 있음

```javascript
llet objectArr = [{ name: "이정한" }, { name: "홍길동" }];
...
- 최종정리: 단순한 원시타입의 값은 indexOf 사용 하고 객체타입은 findIndex 사용
